    ...
    public async savePromptToRepository(
        ...
        try {
            ...
            try {
                ...
                if (error.status !== 404) {
                    console.error(`GitHubService: Unexpected error checking file existence: ${error.status} ${error.message}`);
                    throw error;
                }
                // Silently ignore 404 errors - they're expected when creating new files
            }
